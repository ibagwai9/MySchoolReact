                    ...
                    <div class="panel panel-body">
                        ...
                        @if (session('result_uploaded'))
                            <div class="alert alert-success">
                                ...
                                'guardian/view-result')}}">You May View Result Here</a>
                            </div>
                        @endif

                        <div><p>Current Session: {{$academic_session->name}}</P></div>
                        <div><p>Class: {{$guardian->guardianClass->name}} {{$guardian->school->name}}</p></div>

                        <form class="form-horizontal" role="form" method="POST" enctype="multipart/form-data"
                              action="{{ url('guardian/result')}}">
                            <input type="hidden" name="_token" value="{{ csrf_token() }}">

                            <input type="hidden" name="guardian_id" value="{{$guardian->id}}">
                            <input type="hidden" name="class_id" value="{{$guardian->guardianClass->id}}">
                            <input type="hidden" name="school_id" value="{{$guardian->school->id}}">
                            <input type="hidden" name="academic_session" value="{{$academic_session->name}}">
